...
const EtudiantPage = () => {
  ...
  return (
    <div className='etudiant-page-container'>
      <h2>Page Etudiant</h2>
      {fileData.length > 0 ? (
        <div>
          ...
          {fileData.map(file => (
            <div key={file.id}>
              ...
              <img src={`http://localhost:3100/${file.path}`} alt="Image téléversée" style={{ maxWidth: '100px', height: '100px' }} />
              <hr />
            </div>
          ))}
        </div>
      ...
      <button onClick={handleBack}>Deconnexion</button>
    </div>
  );
};
